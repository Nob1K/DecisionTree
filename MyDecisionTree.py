...
class Decision_tree:
    ...
    # use recursion to build up the tree.
    ...
    def generate_tree(self,data,label):
        ...
        if node_entropy < self.min_entropy:
            label2, count = np.unique(label, return_counts=True)
            max = 0
            index = -1
    ...
    def select_feature(self,data,label):
        ...
        for i in range(len(data[0])):
            ...
            cur_entropy = self.combined_metric(left_y,right_y)
    ...
    def combined_metric(self,left_y,right_y):
        ...
        result = 0
        result = len(left_y)/(len(left_y)+len(right_y)) * self.metric(left_y) + len(right_y)/(len(left_y)+len(right_y)) * self.metric(right_y)
        return result

    # compute entropy/gini_index based on the labels
    ...
    def metric(self, label):
        ...
        if self.metricname == 'entropy':
            ...
            for i in range(len(count)):
                result = result + count[i] * math.log2(count[i] + 1e-15)
            result *= (-1)
